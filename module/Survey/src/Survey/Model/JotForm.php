...
class JotForm {
    ...
    public function __construct($apiKey="", $outputType="json", $debugMode=false){
        ...
        define("baseUrl", "http://api.jotform.com");
    ...
    private function _executeHttpRequest($path, $params=array(), $method){
        ...
        if ($http_status != 200) {
            ...
            switch ($http_status) {
                case 400:
                case 404:
                    //throw new Exception($result_obj["message"]);
                break;
                case 401:
                    //throw new Exception("Unauthorized API call");
                break;
                case 503:
                    //throw new Exception("Service is unavailable, rate limits etc exceeded!");
                break;
                
                default:
                    //throw new Exception($result_obj["info"]);
                break;
            }
        }
        ...
        curl_close($ch);
		//print_r($result_obj);
        if ($this->outputType == "json" && isset($result_obj["content"])) {
